...
import { camera, barChart, thumbsDown, thumbsDownOutline, arrowBack } from 'ionicons/icons';
...
export class CosasFeasComponent implements OnInit {
  ...
  constructor() {
    addIcons({ camera, barChart, thumbsDown, thumbsDownOutline, arrowBack });
  }
  ...
  async cargarFotos() {
    this.cargando = true;
    try {
      this.fotos = await this.fotosService.obtenerFotosPorTipo('fea');
    } catch (error) {
  ...
  async tomarFoto() {
    try {
      ...
      if (image.dataUrl) {
        ...
        const file = new File([blob], `fea_${Date.now()}.jpg`, { type: 'image/jpeg' });

        // Subir foto
        const nuevaFoto = await this.fotosService.subirYGuardarFoto(file, 'fea');
  ...
  verGraficos() {
    this.router.navigate(['/grafico-feas']);
  }
